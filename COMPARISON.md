...
| Tools | Channel + product tools | Programmatic tools via REPL | Delegates to provider CLIs | Intercepts / records tool+model I/O | Review engine + gateway |
...
| Multi-agent | Teams / channels | Recursive subagents (RLM children) | Many providers, thread-per-branch | Fork onto other models | Cheap then strong model cascade |
| UI / control | Product UX + bots | Agents View / ACP | Web, desktop, mobile | CLI (`orca record` …) | GitHub Action + console |
| Contrib posture | PRs + ClawSweeper | Discussions first; vouched PRs only | Active issue triage (`accepted`) | Smaller issue surface | Action + recipes |
