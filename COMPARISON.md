...
| Tools | Channel + product tools | Programmatic tools via REPL | Delegates to provider CLIs | Intercepts / records tool+model I/O (+ origin, vision) | Review engine + gateway | Delegates to provider CLIs; approval UX |
...
| Contrib posture | PRs + ClawSweeper | Discussions first; vouched + ENG/RES Linear | Active issue triage (`accepted`) | Fail-loud flags/scrub; vision seam tests | Fail-closed merge gates | Early-stage; open PRs in flight |
